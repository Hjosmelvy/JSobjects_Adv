...
//Task 3: Have the words "Rolling in the Deep" appear on the HTML page in a div tag.    
    $("#task3").replaceWith(library.music.twenty_tens.Adele[0].song);
//Task 4: (project) Create a site that can display songs from an artist specified by a user search.
var userSearchArtist= null ;

var userSearchDecade=null;

$("#searchArtistBtn").click(function(){
    userSearchArtist= $("#searchBarArtist").val();
    userSearchDecade=$("#searchBarDecade").val();
    var numberOfSongs= library.music.userSearchDecade.userSearchArtist.song.length;
    
    for(var i =0;i<numberOfSongs;i++){
 $("#infoBox").append("<p>"+library.music.userSearchDecade.userSearchArtist[i].song+"</p>");
}
    
});
console.log(userSearchArtist);
